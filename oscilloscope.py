...
class tektronix_tds(oscilloscope_t):
    ...
    def trigger_q(self):
        self.substrate.read("TRIGger?")

    def trigger_single(self):
        self.substrate.write(":SINGle")

    def trigger_force(self):
        self.substrate.write("TRIGger FORCe")
    ...
    def trigger_edge(self, source, edge_type):
        ...
        self.substrate.write("TRIGger:MAIn:MODe NORMal")
        self.substrate.write("TRIGger:MAIn:EDGE:SOURce %s" % source)
        self.substrate.write("TRIGger:MAIn:EDGE:SLOPe %s" % edge_type)
...
class rigol_ds1000z_t(oscilloscope_t):
    ...
    def trigger_force(self):
        self.substrate.write(":TFORce")

    def trigger_edge(self, source, edge_type): 
        assert(self.is_channel(source))
        self.substrate.write(":TRIGger:MODe EDGE")
        self.substrate.write(":TRIGger:EDGe:SOURce %s" % source)
        self.substrate.write(":TRIGger:EDGe:SLOPe %s" % edge_type)

    def amplitude(self):
        return self.substrate.read(":MEASure:VAMPlitude?")
...
class dsox1204a(oscilloscope_t, ieee488_t):
    ...
    def ieee_block_bytes(self):
        ...
        i = int(self.substrate.get_byte())
        length = 0
        for i in range(0, i):
            length = length * 10 + int(self.substrate.get_byte())
        self.substrate.remark("Fetching a %u byte block" % length)

        for l in range(0, l):
            yield self.substrate(get_byte())

    def screenshot(self, filename):
        self.substrate.write(":HARDcopy:INKSaver OFF")
        self.substrate.write(":DISPlay:DATA? PNG, COLor")
    ...
    def channel_autoscale(self, channel):
        ...
        self.substrate.write(":AUToscale %s" % channel)

    def channel_scale(self, channel, scale):
        assert(self.is_channel(channel))
        self.substrate.write(":%s:SCALe %fV" % (channel, scale))

    def channel_offset(self, channel, offset):
        assert(self.is_channel(channel))
        self.substrate.write(":%s:OFFSet %fV" % (channel, offset))
    ...
    def trigger_auto(self):
        self.substrate.write(":TRIGger:SWEep AUTO")

    def trigger_normal(self):
        self.substrate.write(":TRIGger:SWEep NORMal")
    ...
    def measure(self, channel):
        assert(self.is_channel(channel))
        self.substrate.write(":MEASure:SOURce %s" % channel)
